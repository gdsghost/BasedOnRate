    ...
    public class TabbedMainActivity : AppCompatActivity, View.IOnClickListener, View.IOnFocusChangeListener, MaterialDialog.IListCallback 
    {
        ...
        protected override void OnActivityResult(int requestCode, Result resultCode, Intent data)
        {
            try
            {
                ...
                if (requestCode == 500) // Add image story
                {
                    Uri uri = data.Data;
                ...
                else if (requestCode == 513) // Add video camera story
                {
                    var filepath = Methods.AttachmentFiles.GetActualPathFromFile(this, data.Data);
                    if (filepath != null)
                    {
        ...
        private void ViewPagerOnPageScrolled(object sender, ViewPager.PageScrolledEventArgs e)
        {
            try
            {
                ...
                if (e.Position == 0) // News_Feed_Tab
                {
                    ...
                    if (AppSettings.ShowAddPostOnNewsFeed && FloatingActionButton.Visibility != ViewStates.Visible)
                    {
                        FloatingActionButton.Tag = "AddPost";
                        FloatingActionButton.SetImageResource(Resource.Drawable.ic_add);
                ...
                else if (e.Position == 3) // Chat_Tab
                {
                    ToolBar.Visibility = ViewStates.Gone;
                    ChatTab.FloatingActionButtonView_Tag();
                    NewsFeedTab?.MainRecyclerView?.StopVideo();
                }
                else if (e.Position == 4) // More_Tab
                {
                    ToolBar.Visibility = ViewStates.Visible;
                    FloatingActionButton.Visibility = ViewStates.Gone;
                    NewsFeedTab?.MainRecyclerView?.StopVideo();
        ...
        private void GetGeneralAppData()
        {
            try
            {
                ...
                if (Methods.CheckConnectivity())
                    PollyController.RunRetryPolicyFunction(new List<Func<Task>> {() => ApiRequest.Get_MyProfileData_Api(this), ApiRequest.LoadSuggestedUser, ApiRequest.LoadSuggestedGroup, ApiRequest.GetMyGroups, ApiRequest.GetMyPages });
        ...
        public void GetOneSignalNotification()
        {
            try
            {
                ...
                else if (notifier == "Chat")
                {
                    ...
                    {
                        var dataUser = ChatTab?.LastChatTab?.MAdapter?.ChatList?.FirstOrDefault(a => a.UserId == userId);
                        ...
                        if (dataUser != null)
                        {
                            ...
                            intent.PutExtra("ColorChat", dataUser.LastMessage.LastMessageClass.ChatColor ?? AppSettings.MainColor);
                    ...
                    {
                        var dataUser = ChatTab?.LastMessagesTab?.MAdapter?.MLastMessagesUser?.FirstOrDefault(a => a.UserId == userId);
                        ...
                        {
                            intent.PutExtra("TypeChat", "LastMessenger");
                            intent.PutExtra("ColorChat", dataUser.ChatColor ?? AppSettings.MainColor);
                            intent.PutExtra("UserItem", JsonConvert.SerializeObject(dataUser));
                        }
                        else
                        {
