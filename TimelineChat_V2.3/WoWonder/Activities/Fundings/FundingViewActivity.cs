    ...
    public class FundingViewActivity : BaseActivity, MaterialDialog.IListCallback, MaterialDialog.ISingleButtonCallback, IPaymentResultWithDataListener
    {
        ...
        protected override async void OnActivityResult(int requestCode, Result resultCode, Intent data)
        {
            try
            {
                ...
                else if (requestCode == 2654 && resultCode == Result.Ok)
                {
                    StartApiService();
                }
            }
            ...
            try
            {
                ...
                { 
                    if (Methods.CheckConnectivity())
                    {
                        ...
                        (int apiStatus, var respond) = await RequestsAsync.Global.RazorPay(p1.PaymentId, "fund", keyValues);
                        if (apiStatus == 200)
                        {
        ...
        private async Task CashFree(string name, string email , string phone)
        {
            try
            {
                if (Methods.CheckConnectivity())
                {
                    ...
                    (int apiStatus, var respond) = await RequestsAsync.Global.InitializeCashFree("fund", AppSettings.CashFreeCurrency, ListUtils.SettingsSiteList?.CashfreeSecretKey ?? "" , ListUtils.SettingsSiteList?.CashfreeMode, keyValues);
                    if (apiStatus == 200)
                    {
        ...
        private async Task FundingPay()
        {
            try
            {
                if (Methods.CheckConnectivity())
                {
                    (int apiStatus, var respond) = await RequestsAsync.Funding.FundingPay(DataObject.Id, CodeName);
                    if (apiStatus == 200)
                    {
                        Toast.MakeText(this, GetText(Resource.String.Lbl_Donated), ToastLength.Long)?.Show(); 
                        StartApiService();
        ...
        private async Task GetFundingById()
        {
            try
            {
                if (Methods.CheckConnectivity())
                {
                    (int apiStatus, var respond) = await RequestsAsync.Funding.GetFundingById(DataObject.Id);
                    if (apiStatus == 200)
                    { 
