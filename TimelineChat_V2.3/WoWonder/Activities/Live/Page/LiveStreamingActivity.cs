...
namespace WoWonder.Activities.Live.Page
{
    ...
    public class LiveStreamingActivity : RtcBaseActivity, IEventHandler , MaterialDialog.IListCallback, PixelCopy.IOnPixelCopyFinishedListener
    {
        ...
        private FrameLayout RootView, MVideoControlLyt;
        ...
        private async void MSendBtnOnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Methods.CheckConnectivity())
                {
                    ...
                    (int apiStatus, var respond) = await RequestsAsync.Comment.CreatePostComments(PostId, text);
                    if (apiStatus == 200)
                    {
        ...
        private async Task CreateLive()
        {
            ...
            if (apiStatus == 200)
            {
                if (respond is AddPostObject result)
                {
                    ...
                    RunOnUiThread(CreateLiveThumbnail); 
                }
            }
            else
            {
        ...
        private void DeleteLiveStream()
        {
            if (!Methods.CheckConnectivity())
                ...
                PollyController.RunRetryPolicyFunction(new List<Func<Task>> { () => RequestsAsync.Posts.DeleteLive(PostId) });
        }
        ...
        private async Task LoadDataComment()
        {
            ...
            if (Methods.CheckConnectivity())
            {
                ...
                (int apiStatus, var respond) = await RequestsAsync.Posts.CheckCommentsLive(PostId, "10", offset);
