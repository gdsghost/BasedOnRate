...
namespace WoWonder.Activities.Comment.Adapters
{
    public class CommentAdapterViewHolder : RecyclerView.ViewHolder, View.IOnClickListener, View.IOnLongClickListener
    {
        ...
        private readonly CommentObjectExtra ReplyCommentObject;
        ...
        public Button RatingText { get; set; }
        public LinearLayout RatingLinearLayout { get; set; }
        public TextView CountRating { get; set; }
            ...
            try
            {
                if (AdapterPosition != RecyclerView.NoPosition)
                {
                    ...
                    if (v.Id == Image.Id)
                        ...
                        PostClickListener.RateCommentClickAsync(new CommentReplyClickEventArgs { Holder = this, CommentObject = item, Position = AdapterPosition, View = MainView });
                } else
                {
                    if (TypeClass == "Reply" && v.Id == RatingLinearLayout?.Id)
                    {
                        PostClickListener.RateCommentClickAsync(new CommentReplyClickEventArgs { Holder = this, CommentObject = ReplyCommentObject, Position = AdapterPosition, View = MainView });
                    }
                }
            }
            catch (Exception e)
