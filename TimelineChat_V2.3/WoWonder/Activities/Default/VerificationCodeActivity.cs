            ...
            try
            {
                if (!string.IsNullOrEmpty(TxtNumber1.Text) && !string.IsNullOrWhiteSpace(TxtNumber1.Text))
                {
                    if (Methods.CheckConnectivity())
                    {
                        ...
                        {
                            (int apiStatus, var respond) = await RequestsAsync.Global.TwoFactorAsync(UserDetails.UserId, TxtNumber1.Text, UserDetails.DeviceId, UserDetails.DeviceMsgId);
                            if (apiStatus == 200)
                            {
                        ...
                        { 
                            (int apiStatus, var respond) = await RequestsAsync.Global.ActiveAccountSmsAsync(UserDetails.UserId, TxtNumber1.Text, UserDetails.DeviceId, UserDetails.DeviceMsgId);
                            if (apiStatus == 200)
                            {
                                if (respond is AuthObject auth)
                                {
