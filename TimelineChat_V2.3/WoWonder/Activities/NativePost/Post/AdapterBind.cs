...
namespace WoWonder.Activities.NativePost.Post
{
    public class AdapterBind  
    {
        ...
        public void BottomPostPartBind(AdapterHolders.PostBottomSectionViewHolder holder ,AdapterModelsClass item)
        {
            try
            {
                //holder.RatingBar.Rating = (float)Convert.ToDouble(item.PostData.AgoraSid);
                //holder.RatingText.Text = item.PostData.AgoraSid;

                item.PostData.Reaction ??= new Reaction();

                holder.RatingBar.Rating = (float)Convert.ToDouble(item.PostData.Reaction.Type);
                if (holder.RatingBar.Rating > 0)
                    holder.RatingText.Text = item.PostData.Reaction.Type;
                else
