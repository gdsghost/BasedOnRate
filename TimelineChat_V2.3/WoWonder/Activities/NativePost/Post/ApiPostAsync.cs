            ...
            try
            {
                ...
                switch (NativeFeedAdapter.NativePostType)
                {
                    ...
                    case NativeFeedType.Saved:
                        ...
                        break;
                    case NativeFeedType.HashTag:
                        (apiStatus, respond) = await RequestsAsync.Posts.GetGlobalPost(AppSettings.PostApiLimitOnScroll, offset, "hashtag", "", WRecyclerView.Hash);
                        break;
                    case NativeFeedType.Popular:
                        (apiStatus, respond) = await RequestsAsync.Posts.GetPopularPost(AppSettings.PostApiLimitOnScroll, offset);
                        break; 
                ...
                else
                {
                    ...
                    int countList = PostCacheList?.Count ?? 0;
                    if (result?.Data?.Count > 0)
                    {
                        result.Data.RemoveAll(a => a.Publisher == null && a.UserData == null);
                    ...
                    try
                    {
                        ...
                        if (aa?.Count > 0)
                        {
                            for (int i = 0; i < aa.Count; i++)
                            {
                                ...
                                var username = aa[i + 1].Value.Replace("href=", "").Replace('"', ' ').Replace(" ", "")
                                    .Replace(Client.WebsiteUrl, "").Replace("\n", "");

                                var data = dataUser.FirstOrDefault(a =>
                                    a.Key?.ToString() == userid && a.Value?.ToString() == username);
                                if (data.Key != null) continue;
                                i++;
                                if (!string.IsNullOrWhiteSpace(userid) && !string.IsNullOrWhiteSpace(username) &&
                                    !dataUser.ContainsKey(userid))
                                {
