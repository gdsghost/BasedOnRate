...
namespace WoWonder.Activities.Chat.Floating
{
    ...
    public class ChatHeadService : JobIntentService, IFloatingViewListener, View.IOnClickListener
    { 
        ...
        private void IconView_Click()
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        var item = MsgTabbedMainActivity.GetInstance()?.LastChatTab?.MAdapter?.ChatList?.FirstOrDefault(a => a.UserId == DataUser.UserId);
                    ...
                    {
                        var item = MsgTabbedMainActivity.GetInstance()?.LastMessagesTab?.MAdapter?.MLastMessagesUser?.FirstOrDefault(a => a.UserId == DataUser.UserId);
                        if (item != null && item.LastMessage.ToId == UserDetails.UserId && item.LastMessage.FromId != UserDetails.UserId)
                        {
                            item.LastMessage.Seen = "1";
                            MsgTabbedMainActivity.GetInstance()?.LastMessagesTab?.MAdapter?.NotifyDataSetChanged();
                        }
                    }
                }
        ...
        private void CreateNotification(Context context)
        {
            try
            {
                ...
                Intent resultIntent = new Intent(context, typeof(MsgTabbedMainActivity));
