        ...
        public async void OnSuccess(Source p0)
        {
            try
            {
                ...
                if (p0.Status.Contains("succeeded"))
                {
                    ...
                    if (PayType == "Funding")
                    {
                        (int apiStatus, var respond) = await RequestsAsync.Funding.FundingPay(Id, Price);
                        if (apiStatus == 200)
                        {
                    ...
                    else if (PayType == "AddFunds")
                    {
                        ...
                        {
                            (int apiStatus, var respond) = await RequestsAsync.Global.SendMoneyWalletAsync(tabbedWallet.SendMoneyFragment?.UserId, tabbedWallet.SendMoneyFragment?.TxtAmount.Text);
                            if (apiStatus == 200)
                            {
