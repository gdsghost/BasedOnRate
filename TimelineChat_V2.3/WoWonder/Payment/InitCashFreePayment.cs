            ...
            public override async void OnPageStarted(WebView view, string url, Bitmap favicon)
            {
                ...
                { 
                    if (url.Contains("requests.php?f=cashfree"))
                    {
                        ...
                        (int apiStatus, var respond) = await RequestsAsync.Global.CashFreeGetStatus(MActivity.CashFreeObject.AppId, ListUtils.SettingsSiteList?.CashfreeSecretKey ?? "", MActivity.CashFreeObject.OrderId, ListUtils.SettingsSiteList?.CashfreeMode);
                        if (apiStatus == 200)
                        {
