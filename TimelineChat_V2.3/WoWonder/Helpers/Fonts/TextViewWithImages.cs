    ...
    public class TextViewWithImages : AppCompatTextView
    {
        ...
        public SpannableString SetClick(TextViewWithImages textViewWithImages , PostDataObject item, SpannableString spendable, AdapterHolders.PostTopSectionViewHolder viewHolder)
        {
            try
            {
                ...
                if (item.GroupRecipientExists != null && item.GroupRecipientExists.Value && !string.IsNullOrEmpty(item.GroupRecipient.GroupName) && spendable.ToString()!.Contains(item.GroupRecipient.GroupName))
                {
                    SetTextColor(spendable, item.GroupRecipient.GroupName, "#888888");
                    spendable = MakeTextClickAble(textViewWithImages, viewHolder, spendable, item.GroupRecipient.GroupName, "group");
                }
